...
export function MarkupProps({ m, t, theme, onUpdate, onDelete, calibration, onParallel, formatAngle, norms, onUpdateNorms }) {
  ...
  return (
    <div>
      ...
      {["line", "angle3", "angle4", "curve", "perp", "parallel", "projDist", "ratio", "sum", "difference", "percentage"].includes(m.type) && <PropRow label="Width" t={t}><input type="range" min="0.5" max="6" step="0.5" value={m.width || 1.5} onChange={e => onUpdate({ width: +e.target.value })} style={{ width: "100%", accentColor: t.acc }} /></PropRow>}
      {m.type === "silhouette" && <><PropRow label="Width" t={t}><input type="range" min="0.5" max="6" step="0.5" value={m.width || 1.5} onChange={e => onUpdate({ width: +e.target.value })} style={{ width: "100%", accentColor: t.acc }} /></PropRow><PropRow label="Dash" t={t}><select value={m.style || "solid"} onChange={e => onUpdate({ style: e.target.value })} style={{ background: t.surf3, border: `1px solid ${t.bdr}`, borderRadius: 4, padding: "3px 6px", color: t.tx, fontSize: 12, width: "100%", fontFamily: "inherit" }}><option value="solid">Solid</option><option value="dashed">Dashed</option><option value="dotted">Dotted</option></select></PropRow><PropRow label="Scale" t={t}><input type="range" min="0.1" max="5" step="0.05" value={m.scale || 1} onChange={e => onUpdate({ scale: +e.target.value })} style={{ width: "100%", accentColor: t.acc }} /></PropRow><PropRow label="Rotation" t={t}><input type="range" min="0" max="360" step="1" value={(m.rotation || 0) * (180 / Math.PI)} onChange={e => onUpdate({ rotation: +e.target.value * (Math.PI / 180) })} style={{ width: "100%", accentColor: t.acc }} /></PropRow><PropRow label="Fill" t={t}><input type="color" value={(m.fillColor || m.color + "22").slice(0, 7)} onChange={e => onUpdate({ fillColor: e.target.value + "22" })} style={{ width: 40, height: 24, padding: 0, border: "none", cursor: "pointer", borderRadius: 4 }} /></PropRow><PropRow label="Show Frame" t={t}><input type="checkbox" checked={m.showFrame !== false} onChange={e => onUpdate({ showFrame: e.target.checked })} style={{ accentColor: t.acc }} /></PropRow><PropRow label="Points" t={t}><div style={{ display: "flex", gap: 6, alignItems: "center" }}><span style={{ fontSize: 11, color: t.tx2 }}>{(m.paths || []).reduce((s, p) => s + p.points.length, 0)} pts</span><span style={{ fontSize: 9, color: t.tx3 }}>Ctrl+click add • Shift+click remove</span></div></PropRow></>}
...
function measTypeLabel(type) {
  const map = { line: "Line", angle3: "Angle", angle4: "Angle", perp: "Perp", polygon: "Polygon", ratio: "Ratio", sum: "Sum", difference: "Diff", percentage: "Pct", projDist: "Dist" };
        ...
        <>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
            {importedTemplates.map(tmpl => (
              <div key={tmpl.name} onClick={() => setSelectedTemplate(tmpl)} style={{ padding: "10px 12px", borderRadius: 8, background: t.surf3, border: `1px solid ${t.bdr}`, cursor: "pointer", display: "flex", alignItems: "center", gap: 8, transition: "border-color 0.15s" }}>
                <div style={{ flex: 1, minWidth: 0 }}>
